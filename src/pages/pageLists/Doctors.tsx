...
export default function Doctors() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === "rating") list.sort((a: any, b: any) => (b.rating || 0) - (a.rating || 0));
    else if (sortBy === "experience") list.sort((a: any, b: any) => (b.experience || 0) - (a.experience || 0));

    return list;
  }, [query, activeSpecialty, sortBy]);

  useEffect(() => setSelected(null), [query, activeSpecialty, sortBy]);
      ...
      <div className="w-full max-w-[1920px] mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div className="flex-1">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#231F53]">Our <span className="text-indigo-600">Doctors</span></h1>
            <p className="text-gray-600 mt-1 text-sm sm:text-base">Skilled specialists across multiple disciplines — book consultations or view profiles.</p>
          </div>

          <div className="flex w-full md:w-auto items-center gap-3">
            <div className="relative flex-1 md:flex-none w-full md:w-[420px]">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search doctors or specialties..."
                className="pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 w-full focus:outline-none focus:ring-2 focus:ring-indigo-100 bg-white text-sm"
        ...
        {/* Grid */}
        <main>
          {/* Responsive grid: 1 → 2 → 3 → 4 → 5 cols depending on breakpoint */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-6">
            {filtered.map((doctor: any, idx: number) => (
              <motion.article
                ...
                className="group bg-white rounded-2xl shadow-md hover:shadow-xl hover:-translate-y-1 transition transform duration-200 cursor-pointer flex flex-col h-full"
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/app/doctor-details/${doctor._id}`)}
                onKeyDown={(e) => e.key === "Enter" && navigate(`/app/doctor-details/${doctor._id}`)}
              >
                {/* Image with responsive aspect ratio */}
                <div className="relative flex-shrink-0">
                  <div className="w-full overflow-hidden rounded-t-2xl bg-gray-100">
                    <div className="w-full" style={{ paddingTop: '66.66%' /* 3:2 ratio */ }}>
                      <img
                        src={doctor.image}
                        alt={doctor.name}
                        loading="lazy"
                        className="absolute inset-0 w-full h-full object-cover transform group-hover:scale-105 transition duration-500 rounded-t-2xl"
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-xs bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full">{doctor.experience ?? "—"} yrs</span>
                    ...
                    <div className="flex items-center gap-2">
                      {/* On small screens make primary actions stacked and full width */}
                      ...
                      <div className="sm:hidden w-full flex flex-col gap-2">
                        ...
                        <button
                          ...
                          className="w-full px-4 py-2 border border-indigo-600 text-indigo-600 rounded-lg text-sm"
                        >
                          Book
                        </button>
                      </div>
        ...
        {/* Selected modal / drawer: mobile = full-screen bottom sheet, md+ = centered modal */}
        {selected && (
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
            ...
            <motion.div
              ...
              exit={{ y: 50, opacity: 0 }}
              className="relative bg-white w-full md:w-[min(900px,90%)] h-[90vh] md:h-auto md:max-h-[90vh] overflow-auto md:rounded-lg shadow-xl flex flex-col"
              role="dialog"
              aria-modal="true"
            >
              <div className="flex items-start gap-4 p-4 border-b sticky top-0 bg-white z-10">
                <img src={selected.image} alt={selected.name} className="h-16 w-16 object-cover rounded-lg flex-shrink-0" />
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-600 mt-1">{selected.experience ?? "—"} yrs • {selected.degree ?? ""}</div>
                </div>
